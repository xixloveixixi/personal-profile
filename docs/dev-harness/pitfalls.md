...
## Vercel 构建期访问外部运行时 API 导致部署失败

### 场景

GitHub checks 中两个 Vercel 部署均失败。本地将 `NEXT_PUBLIC_API_BASE` 指向不可用地址后，`npm run build` 复现为 `Failed to collect page data for /portfolio/[slug]`；继续排查发现博客详情页也会在 `generateStaticParams()` 中访问 Notion API，网络不可用时会在构建期失败。

### 原因

`app/portfolio/[slug]/page.tsx` 的 `generateStaticParams()` 会在构建阶段调用 Go 后端 `/api/public/projects`。Vercel 构建环境没有本机 Go 服务，`localhost:8080` 或不可访问的 API Base 会直接让构建失败。`app/blog/[slug]/page.tsx` 已经声明 `force-dynamic`，但仍保留 `generateStaticParams()`，导致构建期依赖 Notion API。

### 解决方案

- 公开页列表数据允许构建/运行时降级：`getPublicProjects().catch(() => [])`、`getPublicTimeline().catch(() => [])`。
- portfolio 详情页的 `generateStaticParams()` 失败时返回空数组，避免构建期依赖 Go 后端。
- blog 详情页删除不必要的 `generateStaticParams()`，让 `force-dynamic` 页面在运行时读取 Notion。
- 用不可用 API 地址做验证：`NEXT_PUBLIC_API_BASE=http://127.0.0.1:59999 npm run build` 必须通过。

### 下次如何避免

凡是部署在 Vercel 的 App Router 页面，`generateStaticParams()` 和构建期静态数据收集不能依赖本机服务或不稳定外部 API。需要动态数据时优先使用 `force-dynamic` + 运行时 fetch，并给公开页面提供空态降级。
